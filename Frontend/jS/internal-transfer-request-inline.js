// Internal Transfer Request Inline Script - CSP-friendly, with profile auto-fill
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function loadAndFillProfileData(authUser) {
    try {
      ...
      console.log('📋 Auto-filling internal transfer form with profile data:', profile);
      ...
      // Auto-fill department (current department)
      const empDeptField = document.getElementById('empDept');
      const currentDeptField = document.getElementById('currentDepartment');
      if (profile.app_users_department_name || profile.department_name) {
        const deptValue = profile.app_users_department_name || profile.department_name;
        if (empDeptField) empDeptField.value = deptValue;
        if (currentDeptField) currentDeptField.value = deptValue;
      }
      
      // Auto-fill job title (current position)
      const empTitleField = document.getElementById('empTitle');
      const currentPositionField = document.getElementById('currentPosition');
      if (profile.app_users_job_title || profile.job_title_name_ar || profile.position) {
        const titleValue = profile.app_users_job_title || profile.job_title_name_ar || profile.position;
        if (empTitleField) empTitleField.value = titleValue;
        if (currentPositionField) currentPositionField.value = titleValue;
      }
      
      console.log('✅ Internal transfer form auto-filled successfully');
      
    } catch (error) {
      console.error('❌ Error loading profile data for internal transfer form:', error);
  ...
  document.addEventListener('click', function(e) {
    ...
    try {
      switch (action) {
        case 'submit-transfer': handleTransferSubmit(); break;
        case 'cancel-form':
  ...
  async function handleTransferSubmit() {
    const formData = {
      ...
      budgetImpact: document.getElementById('budgetImpact')?.value || '',
      transferReason: document.getElementById('transferReason')?.value || '',
      skillsMatch: document.getElementById('skillsMatch')?.value || '',
      trainingNeeded: document.getElementById('trainingNeeded')?.value || '',
      requestNotes: document.getElementById('requestNotes')?.value || ''
    };

    // validate
    if (!formData.empFirstName || !formData.empEmail || !formData.currentDepartment || 
        !formData.currentPosition || !formData.targetDepartment || !formData.targetPosition) {
      window.showError && window.showError('الرجاء تعبئة الحقول المطلوبة');
      return;
    }

    // Validate required fields before sending
    if (!formData.empEmail || !formData.transferType || !formData.transferReason || !formData.effectiveDate) {
      window.showError && window.showError('يرجى تعبئة جميع الحقول المطلوبة');
      return;
    }

    // Validate effectiveDate is in the future or present
    if (formData.effectiveDate) {
      const effectiveDate = new Date(formData.effectiveDate);
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      if (effectiveDate < today) {
        window.showError && window.showError('تاريخ السريان يجب أن يكون اليوم أو في المستقبل');
        return;
      }
    }
    ...
    const scheduleOffline = (key, optimisticId) => {
      try {
        ...
        if (window.scheduleSync) {
          window.scheduleSync({ type: 'create', endpoint: '/employee/requests/internal-transfer', method: 'POST', data: payload, key, optimisticId });
        }
        const onFailed = (ev) => {
    ...
    try {
      if (window.apiClient) {
        const response = await window.apiClient.makeRequest('/internal-transfer', {
          method: 'POST',
        ...
        if (response && response.success) {
          ...
          window.addNotification(`تم تقديم طلب #${response.data.reference_number} (نقل داخلي)`, {
            recipient: formData.empEmail,
            type: 'internal_transfer_submitted', 
          ...
          const key = 'requestsInternalTransfer';
          const optimisticId = 'it_' + Date.now();
      ...
      console.error('Internal transfer request submission error:', error);
  ...
  console.log('✓ Internal transfer request inline script loaded');
});
