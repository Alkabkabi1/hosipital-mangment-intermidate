// Employee Guarantee Public Law
class GuaranteePublicLawRequest {
    constructor() {
        this.currentStep = 1;
        this.data = null;
        this.init();
    ...
    async init() {
        ...
        this.clearDraftData();
        const form = document.getElementById('guaranteePublicLawForm');
        if (form) form.reset();
        this.bindEvents();
    ...
    clearDraftData() {
        localStorage.removeItem('guaranteePublicLawDraft');
    }
    ...
    validate(data) {
        const required = [
            'guarantorIdCard', 'guarantorIdSource', 'guarantorAddress', 'guarantorDepartment',
            ...
            'guaranteedSponsor', 'guaranteedWorkplace', 'caseDetails', 'requestDate'
        ];
    ...
    submit() {
        ...
        this.data = {
            ...
            serviceType: 'كفالة غرم وأداء في الحق العام',
            ...data,
    ...
    updatePrintView() {
        ...
        set('p_guaranteedWorkplace', d.guaranteedWorkplace);
        ...
        set('p_guarantorIdNumber', d.guarantorIdCard || '—');
        set('p_guarantorSignature', d.guarantorIdCard || '__________');
        
        set('p_witnessName', d.witnessName || '—');
        set('p_witnessId', d.witnessId || '—');
        set('p_witnessSignature', d.witnessName || '__________');
    ...
    saveToInbox() {
        ...
        const guaranteeRequests = JSON.parse(localStorage.getItem('guaranteePublicLawRequests') || '[]');
        ...
        localStorage.setItem('guaranteePublicLawRequests', JSON.stringify(guaranteeRequests));
    }
    ...
    async loadProfileData() {
        try {
            ...
            console.log('✅ Auto-filled guarantee public law form from profile');
        } catch (error) {
...
function clearGuaranteePublicLawForm() {
    if (window.guaranteePublicLawRequest) {
        window.guaranteePublicLawRequest.clear();
    }
}

document.addEventListener('DOMContentLoaded', () => {
    window.guaranteePublicLawRequest = new GuaranteePublicLawRequest();
});
