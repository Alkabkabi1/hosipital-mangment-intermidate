// Admin Clearance Inbox JavaScript
// Handles the clearance requests inbox page
...
async function loadClearanceRequests() {
  try {
    console.log('🔄 Loading ALL clearance requests using DASHBOARD approach...');
    ...
    // Filter for clearance requests only
    const clearanceRequests = Array.isArray(allApiRequests) ? allApiRequests.filter(r => r.type === 'clearance') : [];
    
    console.log('📊 Clearance requests found:', clearanceRequests.length);
    console.log('📊 Sample clearance data:', clearanceRequests[0]);
    
    allRequests = clearanceRequests;
    ...
    allRequests = JSON.parse(localStorage.getItem('requestsClearance') || '[]');
    filteredRequests = [...allRequests];
    updateKPIs();
    renderTable();
  }
...
function updateKPIs() {
  ...
  console.log('📊 Clearance Inbox KPIs:', { pending, completed, rejected, total });
...
function openDetails(requestId) {
  window.location.href = `admin-clearance-detail.html?id=${requestId}`;
}
...
// Export functions
function exportCSV() {
  const headers = ['الرقم', 'الموظف', 'القسم', 'الحالة', 'تاريخ الطلب', 'آخر يوم عمل', 'السبب'];
  ...
  downloadFile(csvContent, 'clearance_requests.csv', 'text/csv');
...
function exportPDF() {
  // PDF export functionality can be added later
